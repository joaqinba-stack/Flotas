...
import { getAlert, acknowledgeAlert } from "@/lib/data/alerts";
...
export const GET = apiHandler<Ctx>(async (_req, { params }) => {
  ...
  return json(await getAlert(session, id));
});

export const PATCH = apiHandler<Ctx>(async (req, { params }) => {
  const session = await requireApiSession(Role.SUPERVISOR, Role.DESK_AGENT);
  const { id } = await params;
  const input = alertAcknowledgeSchema.parse(await req.json());
  return json(await acknowledgeAlert(session, id, input.status));
});
